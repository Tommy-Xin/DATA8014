...
class TiTokEncoder(nn.Module):
    ...
    """
    def forward(self, pixel_values, latent_tokens):
        batch_size = pixel_values.shape[0]  # Shape: (batch_size, channels, height, width)
        x = pixel_values  # Shape: (batch_size, channels, height, width)
        
        # Step 1: Patch Embedding
        x = self.patch_embed(x)
        print("Step 1: Patch Embedding - x shape:", x.shape)
        x = x.reshape(x.shape[0], x.shape[1], -1)
        print("Step 1.1: Reshape - x shape:", x.shape)
        x = x.permute(0, 2, 1)  # Shape = [*, grid ** 2, width]
        print("Step 1.2: Permute - x shape:", x.shape)
        
        # Step 2: Class and Positional Embeddings
        x = torch.cat([_expand_token(self.class_embedding, x.shape[0]).to(x.dtype), x], dim=1)
        print("Step 2: Class and Positional Embeddings - x shape:", x.shape)
        x = x + self.positional_embedding.to(x.dtype)  # Shape = [*, grid ** 2 + 1, width]
        print("Step 2.1: Add Positional Embedding - x shape:", x.shape)
        
        # Step 3: Process Latent Tokens
        latent_tokens = _expand_token(latent_tokens, x.shape[0]).to(x.dtype)
        print("Step 3: Expand Latent Tokens - latent_tokens shape:", latent_tokens.shape)
        latent_tokens = latent_tokens + self.latent_token_positional_embedding.to(x.dtype)
        print("Step 3.1: Add Positional Embedding - latent_tokens shape:", latent_tokens.shape)
        x = torch.cat([x, latent_tokens], dim=1)
        print("Step 3.2: Concatenate - x shape:", x.shape)
        
        # Step 4: Pre-Layer Normalization
        x = self.ln_pre(x)
        print("Step 4: Pre-Layer Normalization - x shape:", x.shape)
        
        # Step 5: Permute for Transformer
        x = x.permute(1, 0, 2)  # NLD -> LND
        print("Step 5: Permute for Transformer - x shape:", x.shape)
        
        # Step 6: Transformer Layers
        for i in range(self.num_layers):
            x = self.transformer[i](x)
        print("Step 6: Transformer Layers - x shape:", x.shape)
        
        # Step 7: Permute Back
        x = x.permute(1, 0, 2)  # LND -> NLD
        print("Step 7: Permute Back - x shape:", x.shape)
        
        # Step 8: Extract Latent Tokens
        latent_tokens = x[:, 1+self.grid_size**2:]
        print("Step 8: Extract Latent Tokens - latent_tokens shape:", latent_tokens.shape)
        latent_tokens = self.ln_post(latent_tokens)
        print("Step 8.1: Post-Layer Normalization - latent_tokens shape:", latent_tokens.shape)
        
        # Step 9: Fake 2D Shape and Reshape
        if self.is_legacy:
            latent_tokens = latent_tokens.reshape(batch_size, self.width, self.num_latent_tokens, 1)
        else:
            latent_tokens = latent_tokens.reshape(batch_size, self.num_latent_tokens, self.width, 1).permute(0, 2, 1, 3)
        print("Step 9: Fake 2D Shape and Reshape - latent_tokens shape:", latent_tokens.shape)
        
        # Step 10: Convolutional Output
        latent_tokens = self.conv_out(latent_tokens)
        print("Step 10: Convolutional Output - latent_tokens shape:", latent_tokens.shape)
        latent_tokens = latent_tokens.reshape(batch_size, self.token_size, 1, self.num_latent_tokens)
        print("Step 10.1: Reshape - latent_tokens shape:", latent_tokens.shape)
