...
from CRATE.crate_ae.crate_encoder import Block_CRATE_encoder
from CRATE.crate_ae.crate_decoder import Block_CRATE_decoder
...
class TiTokEncoder(nn.Module):
    def __init__(self, config):
        ...
        # CREATE param
        self.crate_dim      = config.crate_config.crate_embed_dim, 
        self.crate_heads    = config.crate_config.crate_num_heads
        self.crate_dim_head = config.crate_config.crate_embed_dim // config.crate_config.crate_num_heads, 
        self.crate_lambd    = config.crate_config.crate_lambd
        self.crate_depth    = {
                                "small": 4,
                                "base": 6,
                                "large": 12,
        ...
        for i in range(self.crate_depth):
            self.transformer.append(Block_CRATE_encoder(
                dim = self.width,
                num_heads = self.crate_heads,
                dim_head = self.crate_dim_head,
                lambd = self.crate_lambd
            ))
...
class TiTokDecoder(nn.Module):
    def __init__(self, config):
        ...
        for i in range(self.crate_depth):
            self.transformer.append(decoder_block(
                dim=self.width, 
                heads=self.crate_heads, 
                dim_head=self.crate_dim_head
                )
            )
